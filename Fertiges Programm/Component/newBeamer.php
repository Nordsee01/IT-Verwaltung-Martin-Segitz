...
if (count($_POST) > 0) {
    ...
    if (!isset($_POST["ANSILumen"])) {
        $newComponentIsValid = false;
    }
    if (!isset($_POST["Eingang"])) {
        $newComponentIsValid = false;
    }
    if (!isset($_POST["Lautsprecher"])) {
        $newComponentIsValid = false;
    }
} else {
    $newComponentIsValid = false;
...
if ($newComponentIsValid) {
    ...
    $insertManager->komponenteHinzufuegen($_POST["Bezeichnung"], $_POST["Raum"], $_POST["Hersteller"], $_POST["Einkaufsdatum"], $_POST["Gewaehrleistungsdauer"], $_POST["Notiz"], $_POST["Hersteller"], 6);

    $komponenten_k_id = $DBSelect->getComponentIDWhere($_POST["Bezeichnung"], $_POST["Raum"], $_POST["Hersteller"], $_POST["Einkaufsdatum"], $_POST["Gewaehrleistungsdauer"], $_POST["Notiz"], $_POST["Hersteller"], 6);

    $insertManager->khaHinzufuegen($komponenten_k_id, 1, $_POST["Seriennummer"]);
    $insertManager->khaHinzufuegen($komponenten_k_id, 14, $_POST["ANSILumen"]);
    $insertManager->khaHinzufuegen($komponenten_k_id, 15, $_POST["Eingang"]);
    $insertManager->khaHinzufuegen($komponenten_k_id, 16, $_POST["Lautsprecher"]);
}
?>
...
<body>
    ...
    <div class="container">
        ...
        <form method="post">
            ...
            <div class="form-group">
                <label for="ANSILumen">ANSI-Lumen</label>
                <input name="ANSILumen" type="text" class="form-control" id="ANSILumen" aria-describedby="ANSILumen" required>
            </div>
            <div class="form-group">
                <label for="Eingang">Eingang</label>
                <input name="Eingang" type="text" class="form-control" id="Eingang" aria-describedby="Eingang" required>
            </div>
            <div class="form-group">
                <label for="Lautsprecher">Lautsprecher</label>
                <input name="Lautsprecher" type="text" class="form-control" id="Lautsprecher" aria-describedby="Lautsprecher" required>
            </div>
